...
namespace MiniCover.Commands
{
    internal class ResetCommand : BaseCommandLineApplication
    {
        ...
        protected override IEnumerable<IMiniCoverOption> MiniCoverOptions => new IMiniCoverOption[] { _workingDirectoryOption, _coverageHitsFileOption };

        protected override Task<int> Execution()
        {
            var errorsCount = 0;
            Console.WriteLine($"Reset coverage for solution directory: '{_workingDirectoryOption.Value.FullName}' on pattern '{_coverageHitsFileOption.Value}'");
